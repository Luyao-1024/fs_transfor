...
- **连接共享**: 同一服务器(host/port/user)在多个标签页打开时复用同一条
  SSH/SFTP 连接(免重连、免重复认证), 引用计数, 最后一个使用方断开时才真正断线;
  一条连接断开, 所有使用它的标签页保留文件界面并显示"连接已断开"提示,
  点工具栏恢复按钮即可重连并回到原目录; 也可主动暂停连接后随时恢复
- **SSH/SFTP**: 密钥 / 密码 / ssh-agent 认证, 首连指纹确认, known_hosts 校验
- **传输**: 面板间拖拽、右键"传输到对侧"、Ctrl+C/X/V 剪贴板(剪切=移动);
  ...
  同名弹 覆盖/跳过/取消; 同名文件、目录复制进自身、链接别名会被拒绝并说明原因
...
SFTP 合约测试使用模拟后端，不能替代真实服务器上的认证、网络和原子覆盖验证。
